...
namespace DomainLayer.Validations.DataValidations.Validations
{
    ...
    internal class SubFieldValidationBuilder : BaseDataValidationBuilder
    {
        ...
        public SubFieldValidationBuilder(Record marcRecord, ValidationBase rules) : base(marcRecord, rules)
        {
            ...
            _fields = Record.GetDataFields(_subFieldValidation.SubField.Parent.Tag.ToString(),
                                        _subFieldValidation.SubField.Parent.Indicator1,
                                        _subFieldValidation.SubField.Parent.Indicator2)?
                                        .SelectMany(x => x.GetSubfields()
                                                    .Where(y => _subFieldValidation.SubField.Code.Contains(y.Code)))
                                        .ToList() ?? [];
        }
